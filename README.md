...
focus: [AWS, Kubernetes, Terraform, CI/CD, GitOps, Observability]
currently: Seeking entry-level DevOps / Cloud Engineering opportunities
fun_fact: I automate things so I don't have to click them twice 🔁
```
...
|---|---|---|
| 🛒 **E-Commerce Microservices on EKS** | Cloud-native e-commerce app with 19 microservices, ArgoCD GitOps, and full observability stack | `EKS` `ArgoCD` `GitHub Actions` `EFK` `Prometheus` `Grafana` |
| 📈 **Monitoring Stack** | End-to-end monitoring and alerting setup for containerized workloads | `Prometheus` `Grafana` `Docker` `Kubernetes` |
| ☁️ **AWS Infrastructure with Terraform** | Automated, modular AWS infra provisioning using Infrastructure as Code | `Terraform` `AWS` `IAM` `VPC` |
